...
export default {
    ...
    return {
      ...
      rules: {
        userName: [
          ...
          { min: 2, max: 20, message: '用户名称长度必须介于 2 和 30 之间', trigger: 'blur' }
        ],
        nickName: [
  ...
  methods: {
    ...
    getList() {
      ...
      listUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          ...
          this.users.forEach((item) => {
            item.avatar = (item.avatar === '' || item.avatar == null) ? require('@/assets/images/profile.jpg') : process.env.VUE_APP_BASE_API + item.avatar
          })
    ...
    /** 重置密码按钮操作 */
    handleResetPwd(row) {
      this.$prompt('请输入 ID = ' + row.userId + ' & 账号为"' + row.userName + '"的新密码', '提示', {
