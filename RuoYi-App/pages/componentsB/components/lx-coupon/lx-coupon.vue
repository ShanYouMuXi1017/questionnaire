<template>
	<view>
		<view class="coupon-item" :class="status !== 0 ? 'h' : ''">
			<view class="coupon-item-left">
				<view class="coupon-item-info">
					<view class="coupon-item-money">¥{{ couponValue }}</view>
					<view class="coupon-item-ask">{{ couponThreshold == 0 ? '无门槛' : '满' + couponThreshold + '元可用' }}</view>
				</view>
				<view class="coupon-item-content">
					<view class="coupon-left-top">
						<view class="coupon-item-name">{{ couponName }}</view>
						<view class="coupon-item-tip">订单结算支付时可用</view>
					</view>
					<view class="coupon-left-middle">
						<view class="coupon-item-isUsed">使用时间:{{ isUsed == '0' ? '暂未使用' :  usageTime}}</view>
					</view>
					<view class="coupon-left-bottom">
						<view class="coupon-item-time">有效期至 {{ endTime }}</view>
					</view>
				</view>
			</view>

			<view class="coupon-item-right" v-if="status === 0">
				<view class="coupon-item-botton" @click="userEvent">去维修</view>
			</view>
		</view>
	</view>
</template>
