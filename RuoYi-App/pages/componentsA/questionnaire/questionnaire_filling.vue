...
export default {
	...
	computed: {
		...
		firstRouteName() {
			...
			return routes.length > 0 ? routes[0] + '—' + routes[routes.length - 1] : this.routeObject.routeName;
		},
		// 提取其他途径
		otherRoutes() {
			const routes = this.routeObject.routeName ? this.routeObject.routeName.split('—') : [];
			return routes.slice(1, routes.length - 1); // 返回除了第一个和最后一个以外的所有途径
		}
	},
	onLoad(options) {
	...
	methods: {
		...
		previewImage() {
		  wx.previewImage({
			current: this.routeObject.imageUrl, // 当前显示的图片链接
			urls: [this.routeObject.imageUrl],  // 图片数组
		  });
		},
	}
};
